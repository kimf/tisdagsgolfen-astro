...
function oldStyleLeaderboardItems(sessions: ScoringSessionItem[], players: Profile[]) {
  ...
  for (const player of players) {
    ...
    const emptyPoints = Array(Math.max(0, 4 - points_array.length)).fill(0);
...
const oldCachedBuilder = cacheable((sessions: ScoringSessionItem[], players: Player[]) =>
  oldStyleLeaderboardItems(sessions, players)
);

export default oldCachedBuilder;
