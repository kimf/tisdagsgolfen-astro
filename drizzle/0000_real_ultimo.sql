...
CREATE UNIQUE INDEX `course_club_name` ON `courses` (`club`,`name`);--> statement-breakpoint
CREATE TABLE `holes` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
...
CREATE UNIQUE INDEX `hole_course_index` ON `holes` (`course_id`,`index`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `profiles_full_name_unique` ON `profiles` (`full_name`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `scorecard_players_player_scorecard` ON `scorecard_players` (`player_id`,`scorecard_id`);--> statement-breakpoint
